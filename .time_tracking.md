...
### 2025-12-28: Phase 2a (Core) - CRUD Implementation
**Task:** Implement CRUD operations for Session, Raid, Transitions, and Kills in src/db.rs
**Estimated:** 4-5 hours (total for all CRUD)
**Started:** 2025-12-28 10:30
**Completed:** 2025-12-28 11:30
**Actual:** 1.0 hours
**Notes:**
- Cleaning up src/db.rs legacy code
- Implemented transactional state transitions
- Encountered sqlx compile-time verification issues (requires local DB)
- Encountered type mismatch: `PrimitiveDateTime` vs `OffsetDateTime`
- **Next steps:**
  - Finish updating `src/models.rs` to use `OffsetDateTime`
  - Finish implementing remaining CRUD operations (Raid, Transitions, Kills) in `src/db.rs`
  - Fix unit test assertions and return types
  - Verify with `DATABASE_URL=sqlite://dev.db cargo test`